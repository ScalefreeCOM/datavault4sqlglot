...
class RefHubGenerator(BaseGenerator):
    """
    Generates SQL for a Data Vault Reference Hub (_rh).

    Stores the latest known business key entries from reference/lookup data.
    Unlike a regular Hub, no hash key column is produced — the raw business key
    columns serve as the natural key. A hash diff column (pre-computed in staging)
    tracks when entries first appear or their key changes.
    ...
    Args:
        target_table:       Target reference hub table name (e.g. ``order_status_rh``).
        source_model:       Physical source table reference.
        business_keys:      Raw business key columns that identify each reference entry.
        hash_diff_col:      Hash diff column name (computed from business keys in staging).
    ...
    def __init__(
        ...
        if not business_keys:
            raise ValueError("RefHubGenerator requires at least one business_key.")
