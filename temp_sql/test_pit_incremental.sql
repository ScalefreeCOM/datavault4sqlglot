-- PIT -- Incremental — existing_dimension_keys CTE, NOT IN filter on SDTS

WITH existing_dimension_keys AS (
  SELECT
    DIM_CUSTOMER_KEY
  FROM "DV_DB"."BUSINESS_VAULT"."PIT_CUSTOMER"
), pit_records AS (
  SELECT
    NULLIF(
      LOWER(
  ...
  FROM pit_records
  WHERE
    NOT DIM_CUSTOMER_KEY IN (SELECT
        DIM_CUSTOMER_KEY
    FROM existing_dimension_keys)
  ORDER BY
    SDTS
)
SELECT
